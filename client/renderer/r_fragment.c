...
static int32_t				cm_numMarkPoints;
static int32_t				cm_maxMarkPoints;
static vec3_t			*cm_markPoints;

static int32_t				cm_numMarkFragments;
static int32_t				cm_maxMarkFragments;
static markFragment_t	*cm_markFragments;

static cplane_t			cm_markPlanes[MAX_FRAGMENT_PLANES];

static int32_t				cm_markCheckCount;


int32_t PlaneTypeForNormal (const vec3_t normal)
{ 
	if (normal[0] == 1.0)  
		return 0;//PLANE_X;  
	if (normal[1] == 1.0)  
...
float *worldVert (int32_t i, msurface_t *surf)
{
	int32_t e = r_worldmodel->surfedges[surf->firstedge + i];
	if (e >= 0)
...
=================
*/
static void R_ClipFragment (int32_t numPoints, vec3_t points, int32_t stage, markFragment_t *mf)
{
	int32_t			i;
	float		*p;
	qboolean	frontSide;
	...
	int32_t			sides[MAX_FRAGMENT_POINTS];
	cplane_t	*plane;
