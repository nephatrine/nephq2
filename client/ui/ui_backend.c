...
static	void	Slider_DoSlide( menuslider_s *s, int32_t dir );
...
int32_t mouseOverAlpha (menucommon_s *m)
{
...
{
	int32_t alpha = mouseOverAlpha(&a->generic);
...
{
	int32_t i, alpha = mouseOverAlpha(&f->generic), xtra;
	char tempbuffer[128]="";
	int32_t offset;
	...
	// add cursor thingie
	if ( Menu_ItemAtCursor(f->generic.parent)==f  && ((int32_t)(Sys_Milliseconds()/250))&1 )
...
{
	const char **n;
	int32_t y = 0, alpha = mouseOverAlpha(&l->generic);
...
{
	int32_t alpha = mouseOverAlpha(&s->generic);
...
void Slider_DoSlide (menuslider_s *s, int32_t dir)
{
	s->curvalue += dir;
...
{
	int32_t	i, alpha = mouseOverAlpha(&s->generic);
...
void SpinControl_DoSlide (menulist_s *s, int32_t dir)
{
	if (!s->itemnames || !s->numitemnames)
		return;
...
{
	int32_t height = ((menucommon_s *) menu->items[menu->nitems-1])->y + 10;
	...
	else if (item && item->type != MTYPE_FIELD)
	{
		if (item->flags & QMF_LEFT_JUSTIFY)
		{
			SCR_DrawChar (menu->x+item->x+item->cursor_offset-24, menu->y+item->y,
						ALIGN_CENTER, 12+((int32_t)(Sys_Milliseconds()/320)&1),
...
extern	const char *(*m_keyfunc) (int32_t key);
...
=================
*/
int32_t Slider_CursorPositionX (menuslider_s *s)
{
	float range;
	...
	return (int32_t)(SCR_ScaledVideo(MENU_FONT_SIZE) + RCOLUMN_OFFSET + (SLIDER_RANGE)*SCR_ScaledVideo(MENU_FONT_SIZE) * range);
}
...
=================
*/
int32_t NewSliderValueForX (int32_t x, menuslider_s *s)
{
