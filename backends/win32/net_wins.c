...
qboolean	NET_StringToSockaddr (char *s, struct sockaddr *sadr)
{
	...
	if ((strlen(s) >= 23) && (s[8] == ':') && (s[21] == ':'))	// check for an IPX address
	{
		...
		DO(19, sa_nodenum[5]);
		sscanf (&s[22], "%u", &val);
		((struct sockaddr_ipx *)sadr)->sa_socket = htons((uint16_t)val);
	}
	else
	{
		...
		{
			*(int32_t *)&((struct sockaddr_in *)sadr)->sin_addr = inet_addr(copy);
		}
		else
		{
			if (! (h = gethostbyname(copy)) )
				return 0;
			*(int32_t *)&((struct sockaddr_in *)sadr)->sin_addr = *(int32_t *)h->h_addr_list[0];
		}
	}
...
====================
*/
int32_t NET_IPXSocket (int32_t port)
{
	int32_t					newsocket;
