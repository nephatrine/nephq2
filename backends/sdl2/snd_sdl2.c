...
#include "../../client/sound/include/local.h"
...
SDL_Cache(sfx_t *sfx, wavinfo_t *info, byte *data)
{
	...
	sc = sfx->cache = (sfxcache_t*)Z_TagMalloc(len + sizeof(sfxcache_t), TAG_AUDIO);
 ...
 */
static void
SDL_Callback(void *data, uint8_t *stream, int length)
{
	int length1;
	int length2;
...
/*
	...
	*/
	...
	backend->buffer = (unsigned char*)SDL_calloc(1, samplesize);
